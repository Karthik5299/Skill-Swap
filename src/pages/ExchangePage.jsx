...
const ExchangePage = () => {
  ...
  const handleSendRequest = async (requestData) => {
    try {
      ...
      const recipientDoc = await getDoc(doc(db, "users", userProfile.uid));
      ...
      const exchangeData = {
        ...
        // Requester (Teacher in this context if they're teaching)
        requesterId: currentUser.uid,
        requesterName: requesterData?.displayName || currentUser.displayName,
        requesterEmail: requesterData?.email || currentUser.email,
        requesterPhoto: requesterData?.photoURL || currentUser.photoURL,
        requesterSkillsToTeach: requesterData?.skillsToTeach || [],
        requesterSkillsToLearn: requesterData?.skillsToLearn || [],
        
        // Recipient (Learner in this context if they're learning)
        recipientId: userProfile.uid,
        recipientName: recipientData?.displayName || userProfile.displayName,
        recipientEmail: recipientData?.email || userProfile.email,
        recipientPhoto: recipientData?.photoURL || userProfile.photoURL,
        recipientSkillsToTeach: recipientData?.skillsToTeach || [],
        recipientSkillsToLearn: recipientData?.skillsToLearn || [],
      ...
      batch.set(recipientNotificationRef, {
        ...
        message: `${requesterData?.displayName || currentUser.displayName} wants to exchange skills with you on ${requestData.date} at ${requestData.time}`,
        ...
        senderName: requesterData?.displayName || currentUser.displayName,
        senderPhoto: requesterData?.photoURL || currentUser.photoURL,
        date: requestData.date,
        time: requestData.time,
      });
      ...
      batch.set(requesterNotificationRef, {
        ...
        message: `Your exchange request with ${recipientData?.displayName || userProfile.displayName} has been sent for ${requestData.date} at ${requestData.time}`,
        type: "exchange_sent",
        meetingLink,
        relatedId: exchangeRef.id,
        exchangeId: exchangeRef.id,
      ...
      toast.success(`Exchange request sent to ${recipientData?.displayName || userProfile.displayName}!`);
