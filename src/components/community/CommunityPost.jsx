...
// ── Reusable avatar: shows photo if available, else coloured initial ──────────
const Avatar = ({ photo, name, size = "h-12 w-12" }) => {
  ...
  if (photo && !imgError) {
    return (
      <img
        key={photo}
        className={`${size} rounded-full object-cover shadow-md flex-shrink-0`}
        src={photo}
        alt={name}
  ...
  return (
    <div
      className={`${size} rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-md flex-shrink-0`}
    >
      <span className={size === "h-10 w-10" ? "text-xs" : "text-sm"}>
        {initial}
      </span>
    </div>
  );
      ...
      <div className="p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-4">
            <Avatar photo={post.authorPhoto} name={post.authorName} size="h-12 w-12" />
              ...
              {post.comments?.length > 0 ? (
                [...post.comments]
                  ...
                  .map((comment, index) => (
                    ...
                    >
                      <Avatar photo={comment.userPhoto} name={comment.userName} size="h-10 w-10" />
                      <div className="flex-1">
